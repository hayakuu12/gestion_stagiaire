...
<div class="page-header">
    <div><h2><i class="fas fa-edit me-2"></i>تعديل مصلحة</h2></div>
...
<?php $v = array_merge($service, $_SERVER['REQUEST_METHOD']==='POST' ? $_POST : []); ?>

<div class="card" style="max-width:600px">
    <div class="card-header"><i class="fas fa-sitemap me-2"></i><?= htmlspecialchars($service['nom_ar']) ?></div>
    <div class="card-body">
        <form method="POST" action="index.php?action=edit_service&id=<?= $service['id'] ?>">
            <div class="mb-3">
                <label class="form-label">الاسم بالعربية <span class="text-danger">*</span></label>
                <input type="text" name="nom_ar" class="form-control" value="<?= htmlspecialchars($v['nom_ar']) ?>" required>
            </div>
            <div class="mb-4">
                <label class="form-label">Nom en Français</label>
                <input type="text" name="nom_fr" class="form-control" value="<?= htmlspecialchars($v['nom_fr']??'') ?>">
            </div>
